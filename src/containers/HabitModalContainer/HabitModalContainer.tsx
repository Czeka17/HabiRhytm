...
function HabitModalContainer(){
    ...
    if(state.is === 'selection' && SelectedHabitId === null){
        content = <div><button onClick={() => HabitSelectionHandler(true)}>Habit</button><button onClick={() => HabitSelectionHandler(false)}>Addiction</button><Button onClick={handleClose} name={"Exit"}/></div>
    }
    if(state.is === 'name'){
        content = <div>
            <input
